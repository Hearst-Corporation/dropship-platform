    ...
    input_schema: {
      ...
      properties: {
        ...
        design_proposals: {
          type: 'array',
          description:
            'EXACTLY 3 design candidates the operator picks from in the chat. Each is one of the 25 curated **design presets** (5 general-purpose originals plus 20 niche-specific ones, each tagged with a `suitedFor` niche list) plus the primary+accent hex colors you recommend for THIS niche. Prioritize the preset(s) whose `suitedFor` matches this niche when one exists (e.g. pet care → pet-playful, tech gadgets → gadget-graphite); fall back to a general-purpose preset (editorial-serif, tech-mono, brutalist-luxe, gen-z-bold, lifestyle-warm, scandi-minimal, art-deco-glam, brutal-neon, y2k-pastel, mono-architect, botanical-green, coastal-nautical, diner-retro, wabi-sabi, desert-terracotta) when no niche-specific preset fits well or more variety is wanted. ⚠️ The `preset` field here is a DESIGN SYSTEM (typo + structure rules) — NOT a storefront template id. NEVER put template ids like `luxury-mono`, `wellness-soft`, `fiora-locks-wh1270` here — those belong to `suggested_template`. For luxury niches, use `brutalist-luxe` or `editorial-serif` (or `jewel-mono` for jewelry-adjacent niches) as the preset. Choose the 3 presets that best fit the audience and keep them visually distinct (e.g. for "pet care" propose pet-playful + lifestyle-warm + editorial-serif). Each accent must visually contrast its primary. Once the operator picks one in the chat, those exact colors are LOCKED — no other component is allowed to invent new ones.',
          items: {
            type: 'object',
            properties: {
              preset: {
