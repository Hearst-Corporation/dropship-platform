...
export function ChartTooltip({
  ...
  return (
    <div className="rounded border border-zinc-950/10 bg-white px-3 py-2 text-xs text-zinc-950 shadow-lg dark:border-white/10 dark:bg-zinc-800 dark:text-zinc-100">
      {label !== undefined && (
        <div className="mb-1 font-medium text-zinc-500 dark:text-zinc-300">{label}</div>
      )}
      {payload.map((entry, i) => (
...
export function ChartEmptyState({ height }: { height: number }) {
  return (
    <div
      className="flex items-center justify-center rounded-lg border border-zinc-950/10 bg-white text-sm text-zinc-500 dark:border-white/10 dark:bg-zinc-900 dark:text-zinc-400"
      style={{ height }}
    >
