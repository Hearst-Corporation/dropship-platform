 ...
 * Static sidebar on desktop, slide-over on mobile. Dark (gray-900), indigo accent.
 * Scopes the dark theme to /admin/* only — storefront keeps its own chrome.
 */
type NavItem = {
  name: string;
  href: string;
...
function Brand() {
  return (
    <div className="flex h-16 shrink-0 items-center gap-2">
      <span className="flex size-8 items-center justify-center rounded-lg bg-indigo-500 text-sm font-bold text-white">
        H
      </span>
      <span className="text-sm font-semibold tracking-tight text-white">Hearst Merchant</span>
    </div>
  );
}
