  ...
  return (
    <div
      className={cn(
        ...
        step.isError ? 'bg-rose-500/10 text-rose-300' : 'bg-white/5 text-gray-400',
      )}
    >
      {icon}
...
function Bubble({ msg }: { msg: ChatMessage }) {
  ...
  return (
    <div className="space-y-1.5">
      ...
      {msg.text && (
        <div className="max-w-[90%] whitespace-pre-wrap rounded-2xl rounded-bl-sm bg-white/5 px-3 py-2 text-sm text-gray-100 ring-1 ring-white/10">
          {msg.text}
        </div>
      )}
    </div>
    ...
    <>
      ...
      <aside
        className={cn(
          ...
          'border-l border-white/10 bg-gray-900',
          RAIL_WIDTH,
        )}
      >
        <ChatBody />
        ...
        <div className="fixed inset-0 flex justify-end">
          ...
          >
            <button
              ...
              className="absolute right-3 top-3 z-10 rounded-md p-1.5 text-gray-400 hover:bg-white/5 hover:text-white"
              aria-label="Fermer"
            >
