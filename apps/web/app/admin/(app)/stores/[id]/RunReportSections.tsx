 ...
 */
export function RunReportSections({ report }: { report: StoreRunReport | null }) {
  if (!report) return null;

  const suppliersSorted = [...report.suppliers].sort((a, b) => {
    const w = (s: typeof a) =>
      s.productsFound > 0 ? 0 : s.considered ? 1 : s.status === 'automation' ? 2 : 3;
    return w(a) - w(b);
  });
    ...
    <>
      <section className="border-t border-zinc-950/10 pt-8 dark:border-white/10">
        ...
        <DescriptionList className="mt-4 sm:grid-cols-2">
          ...
          <DescriptionDetails>{report.markets.join(' + ') || 'FR'}</DescriptionDetails>
          ...
          <DescriptionDetails>{report.assets.notes || ASSET_STATUS_LABEL[report.assets.status]}</DescriptionDetails>

          {report.brief && (
            <>
              <DescriptionTerm>Brief opérateur</DescriptionTerm>
              <DescriptionDetails className="whitespace-pre-wrap">{report.brief}</DescriptionDetails>
            </>
          )}
        </DescriptionList>
      </section>
              ...
              <TableBody>
                {report.products.map((p) => (
                  <TableRow key={`${p.supplier}-${p.externalId}`}>
                    ...
                    <TableCell>
                      <AdminBadge status={p.status === 'imported' ? 'active' : p.status}>
      ...
      {plan && (
        <section className="border-t border-zinc-950/10 pt-8 dark:border-white/10">
          ...
          <DescriptionList className="mt-4 sm:grid-cols-2">
            ...
            <DescriptionDetails>{plan.dailyBudgetEur} € / jour</DescriptionDetails>
            ...
            <DescriptionDetails>{plan.negativeKeywords.join(' · ')}</DescriptionDetails>
          </DescriptionList>
