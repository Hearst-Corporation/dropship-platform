...
} from "@/lib/suppliers/policy-view";
...
export default async function SettingsPage() {
  ...
  const expiresAt = aliExpires?.value
    ? new Date(parseInt(aliExpires.value))
    : null;
    ...
    <div className="space-y-8">
      ...
      >
        <div className="divide-y divide-zinc-950/10 dark:divide-white/10">
            ...
            {isConnected ? (
              <dl className="grid grid-cols-1 gap-x-6 gap-y-2 sm:grid-cols-2 2xl:grid-cols-3">
                ...
                <Field
                  label="Dernière auth"
                  value={
                    aliToken?.updatedAt ? fmtDate(aliToken.updatedAt) : "–"
                  }
                />
              </dl>
            ) : (
              <Text className="text-sm">
                L&apos;agent a besoin d&apos;un <Code>access_token</Code> OAuth
                pour appeler <Code>aliexpress.solution.product.list.get</Code>.
          ...
          >
            <Text className="text-sm">
              L&apos;authentification CJ nécessite une{" "}
              <Strong>API Key dédiée</Strong> (pas le mot de passe du compte).
              Va sur{" "}
              <TextLink
              ...
              &#8594; Account Settings &#8594; Developer &#8594; copie
              l&apos;API Key et mets-la dans{" "}
      ...
      <Table dense bleed clip>
        ...
        <TableBody className="[&>tr:last-child>td]:border-b-0">
          {rows.map((row) => {
            ...
            const cMeta = CONNECTION_META[row.connectionState] ?? {
