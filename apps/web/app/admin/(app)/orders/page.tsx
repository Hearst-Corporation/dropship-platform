...
export default async function OrdersPage() {
  ...
  const [ordersResult, awaitingResult] = await Promise.all([
    medusa.getOrders({ limit: 50 }).catch((e) => ({
      error: e instanceof Error ? e.message : "Unknown error",
      orders: [] as Awaited<ReturnType<typeof medusa.getOrders>>["orders"],
    })),
    getDbRead().query<{
      medusa_order_id: string;
      supplier_order_id: string;
  ...
  const fetched = await Promise.all(
    missingIds.map((id) => medusa.getOrder(id).catch(() => null)),
  );
  ...
  const stats = {
    paidOrders: orders.filter(
      (o) =>
        o.payment_status === "captured" || o.payment_status === "authorized",
    ).length,
    awaitingPayment: awaitingPayment.length,
    paidAtSupplier: allLegs.filter((f) => f.status === "sent" && f.paid_at)
      .length,
  ...
  const kpis = [
    ...
    {
      label: "Payées fournisseur",
      value: String(stats.paidAtSupplier),
      icon: TruckIcon,
    },
    {
    ...
    <div className="flex min-w-0 flex-1 flex-col space-y-8">
      ...
      {awaitingPayment.length > 0 && (
        <AdminSection
          ...
          actions={
            <Badge color="zinc">
              {awaitingPayment.length} commande
              {awaitingPayment.length > 1 ? "s" : ""}
            </Badge>
          }
          flush
        >
          <div className="px-6 py-4 sm:px-8">
            <Text>
              AE n&apos;a pas d&apos;API de paiement. Ouvre le lien, paie sur
              aliexpress.com, puis clique <Strong>Marquer payée</Strong>.
            ...
            <Table dense bleed clip>
              ...
              <TableBody className="[&>tr:last-child>td]:border-b-0">
                {awaitingPayment.map((row) => {
                  const ageMs =
                    Date.now() - new Date(row.forwarded_at).getTime();
      ...
      {!fetchError && orders.length === 0 && (
        <AdminSection title="Aucune commande pour le moment.">
          <Text>
            Les commandes Medusa payées apparaîtront ici dès qu&apos;un client
            passera commande.
          </Text>
        </AdminSection>
      )}
                          ...
                          {legs.length === 0 ? (
                            ...
                            legs.map((leg, legIdx) => {
                                ...
                                if (leg.supplier === "aliexpress") {
                                  return (
                                    ...
                                    >
                                      <AdminBadge
                                        status={
                                          leg.paid_at ? "paid" : "pending"
                                        }
                                      >
                                        {leg.paid_at ? "payée" : "à payer"}
                                      </AdminBadge>
                                      <TextLink
                                        href={aliExpressOrderUrl(
                                          leg.supplier_order_id,
                                        )}
                                        target="_blank"
                                        rel="noopener noreferrer"
                              ...
                              return (
                                <div
                                  key={legIdx}
                                  className="flex max-w-52 flex-col items-start gap-1"
                                >
                                  <AdminBadge status="error">
                                    erreur ({leg.supplier})
                                  </AdminBadge>
                                  {leg.error_message && (
                                    <Text
