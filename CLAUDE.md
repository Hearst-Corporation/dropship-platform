...
- **Generated assets path**: `asset-generator.ts` picks the storage backend at runtime via `isR2Configured()`. When the five `R2_*` env vars are set (prod on Vercel), ComfyUI/fal output uploads straight to Cloudflare R2 under `{slug}/run-{ts}/{filename}` and `dropship_stores.*_url` columns store the absolute `https://pub-...r2.dev/...` URL — this is the production path, since Vercel's filesystem outside `/tmp` is read-only at runtime. When R2 isn't configured (local dev), it falls back to writing `apps/web/public/generated/{slug}/run-{ts}-flux-kontext/` with a `current/` symlink, exactly as before R2 was wired in.
